...
class App extends Component {
  constructor(){
    ...
    this.state ={
      todos:[
      {id:1,text:'abc',isDone:false},
      {id:2,text:'def',isDone:false}
      ],
      newTodo:{text:''}
    };
  };
...
this.setState(({todos}) => ({
  ...
  render() {
    let remaining= this.state.todos.filter((todo)=>{
      return !todo.isDone
    }
    return (
      <div className="App">
      <h1>My todo
      <span>({this.remaining}/{this.state.todos.length})</span>
      </h1>
      <ul>
      {this.state.todos.map((item,key)=>{
       ...
       <button onClick={this.createNewToDpItem}>+</button>
       </div>
